...
public class enemyMove05 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (flag)
        {
            transform.position = Vector3.MoveTowards(transform.position, new Vector3(4, -4, 1), speed * Time.deltaTime);
        }

        else if (!flag)
        {
            transform.position = Vector3.MoveTowards(transform.position, new Vector3(4, 6, 1), speed * Time.deltaTime);
        }
    }
